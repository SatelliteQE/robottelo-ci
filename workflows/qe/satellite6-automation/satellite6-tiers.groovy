  ...
  stage("Configure robottelo according to tier instance") {
    steps {
      script {
        ...
        Branch = branch_selection("${SATELLITE_VERSION}")
        checkout([$class: 'GitSCM', branches: [[name: "*/${Branch}"]], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'RelativeTargetDirectory', relativeTargetDir: 'robottelodir']], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/SatelliteQE/robottelo']]])
        configFileProvider([configFile(fileId: 'bc5f0cbc-616f-46de-bdfe-2e024e84fcbf', variable: 'CONFIG_FILES')]) {
    ...
    withCredentials([sshUserPrivateKey(credentialsId: 'id_hudson_rsa', keyFileVariable: 'identity', usernameVariable: 'userName')]) {
      script {
        ...
        junit allowEmptyResults: true,
          ...
          testDataPublishers: [[$class: 'ClaimTestDataPublisher'], [$class: 'StabilityTestDataPublisher']],
