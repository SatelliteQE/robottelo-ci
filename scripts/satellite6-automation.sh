...
if [[ "${SAUCE_PLATFORM}" != "no_saucelabs" ]]; then
    ...
    # Zalenium
    sed -i "s/^# webdriver_desired_capabilities=platform=ANY,maxDuration=5400,idleTimeout=1000,start-maximised=true,screenResolution=1600x1200,tags=[${JOB_NAME}]/" robottelo.properties
