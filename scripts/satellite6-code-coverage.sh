...
function python_code_coverage () {
    ...
    ssh $ssh_opts root@"${SERVER_HOSTNAME}" "cd /etc/coverage ; tar -cvf coverage.${ENDPOINT}.tar .coverage.*"

    # Combine the coverage output to a single file and create a xml file.
    ssh $ssh_opts "root@${SERVER_HOSTNAME}" "cd /etc/coverage/ ; coverage combine"
    ...
    scp $ssh_opts -r "root@${SERVER_HOSTNAME}:/etc/coverage/coverage.${ENDPOINT}.tar" .
}
...
function ruby_code_coverage () {
    ...
    ssh $ssh_opts root@"${SERVER_HOSTNAME}" "cd /etc/coverage/ruby/tfm/reports/ ; tar -cvf /root/tfm_reports_${ENDPOINT}.tar ./."

    # Fetch the tfm_reports.${ENDPOINT}.tar file to the project folder.
    scp $ssh_opts -r "root@${SERVER_HOSTNAME}:/root/tfm_reports_${ENDPOINT}.tar" .
}
