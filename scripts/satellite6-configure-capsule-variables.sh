...
SATELLITE_SERVER_HOSTNAME=`echo ${SATELLITE_SERVER_HOSTNAME} | xargs`
...
sed -i "s|SUBNET_MASK=.*|SUBNET_MASK=\"${NETMASK}\"|" satellite6-populate-capsule.sh
sed -i "s|SUBNET_GATEWAY=.*|SUBNET_GATEWAY=\"${GATEWAY}\"|" satellite6-populate-capsule.sh
